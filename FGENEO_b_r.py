...
class GENEO_MLP_Small_b_r(nn.Module):
    def __init__(self, patterns, num_classes):
        ...
        self.Rotational_parameters =  nn.Parameter(torch.rand(len(patterns))*90, requires_grad=True) 

    def forward(self, x):
        # print(self.Rotational_parameters)
        # exit()
...
if __name__ == "__main__":
  ...
  torchvision.transforms.PILToTensor()]
    ...
    patterns = torch.load("/home/jcolombini/GENEO/patterns.pt", weights_only=True)
    Model = GENEO_MLP_Small_b_r(patterns, num_classes=2)
